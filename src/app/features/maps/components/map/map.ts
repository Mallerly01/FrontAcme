...
export class Map implements OnInit {
  ...
  //Guardaremos los servicios nativos de Google aqui un vez importados
  ...
  ngOnInit(){
    this.mapService.loadApi().then(async () => {
    ...
    }).catch(error => console.error('Error critico cargando Google Maps', error));
  }
  ...
  //ACCIÓN 1: EL USUARIO ARRASTRA EL PIN EN EL MAPA (TRADUCCION MEDIANTE GOOGLE GEOCÓDIGO)
  onMarkerDragEnd(event: google.maps.MapMouseEvent){
    if(event.latLng){
      const coordenadas =event.latLng.toJSON();

      this.zone.run(() => {
        //Actualizamos los numeros en tu footer de inmediato
